...
// ─── Mock: replace with real DB flag when available ───────────────────────────
const IS_FIRST_VISIT = true; // set to false to bypass splash
// ──────────────────────────────────────────────────────────────────────────────
...
export default function HomePage() {
  ...
  // Splash state
  const [phase, setPhase] = useState<Phase>(IS_FIRST_VISIT ? "splash" : "done");
  ...
  function dismissSplash() {
    ...
    // Haptic: medium impact — entering the dashboard is a meaningful moment
  ...
  function select(id: string) {
    ...
    // Haptic: selection — discrete tab switch
  ...
  function toggleComplete(id: string) {
    setCompletedIds(prev => {
      ...
      if (wasCompleted) {
        next.delete(id);
        // Haptic: light — undoing a completion is a minor interaction
        haptic.trigger("light");
      } else {
        next.add(id);
        // Haptic: success — completing a guide is a positive achievement
      ...
      >
        ...
        <div className="mx-4 sm:mx-8 mb-4 sm:mb-6 shrink-0">
          ...
          >
            ...
            {/* Card body — click to toggle toggleable row */}
            <div
              onClick={() => {
                haptic.trigger("light");
        ...
        >
          <div className="flex flex-col md:flex-row w-full rounded-2xl border border-primary-blue/10 bg-white/50 backdrop-blur-sm overflow-hidden flex-1 min-h-0">
            ...
            >
              ...
              {/* Animated scroll hint arrow overlay */}
              <div
                className={[
